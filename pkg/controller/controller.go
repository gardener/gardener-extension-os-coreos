...
type operatingSystemConfigReconciler struct {
	logger   logr.Logger
	actuator OperatingSystemConfigActuator
...
// NewOperatingSystemConfigController creates a new reconcile.Reconciler that reconciles
// OperatingSystemConfig resources of Gardener's `extensions.gardener.cloud` API group.
func NewOperatingSystemConfigController(logger logr.Logger, actuator OperatingSystemConfigActuator) reconcile.Reconciler {
...
func (r *operatingSystemConfigReconciler) InjectStopChannel(stopCh <-chan struct{}) error {
	r.ctx = ContextFromStopChannel(stopCh)
	return nil
}
