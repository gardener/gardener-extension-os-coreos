...
import (
	...
	osccontroller "github.com/gardener/gardener-extension-os-coreos/pkg/controller"
	"github.com/gardener/gardener-extension-os-coreos/pkg/version"
...
// NewCommandStartExtensionCoreOS starts the controller-manager for the CoreOS Container Linux config extension
...
func NewCommandStartExtensionCoreOS() error {
	...
	c, err := controller.New(controllerName, mgr, controller.Options{
		...
		Reconciler:              osccontroller.NewOperatingSystemConfigController(log.WithName(controllerName), coreos.NewOperatingSystemConfigActuator(log.WithName(fmt.Sprintf("%s-actuator", controllerName)))),
	})
	if err != nil {
	...
	if err := c.Watch(&source.Kind{Type: &extensionsv1alpha1.OperatingSystemConfig{}}, &handler.EnqueueRequestForObject{}, osccontroller.GenerationChangedPredicate(), coreos.Predicate()); err != nil {
		entryLog.Error(err, "unable to watch OperatingSystemConfig resources")
		return err
	}

	if err := c.Watch(&source.Kind{Type: &corev1.Secret{}}, &handler.EnqueueRequestsFromMapFunc{ToRequests: osccontroller.SecretToOSCMapper(mgr.GetClient())}); err != nil {
